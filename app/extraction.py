...
def extract_features(file_path: Union[str, io.BytesIO]) -> np.ndarray:
    ...
    for idx in range(20):
        features.append(float(np.mean(mfcc[idx])))
        features.append(float(np.var(mfcc[idx])))

    return np.array(features, dtype=float)



def process_audio_folder(folder_path: str, label: Optional[str] = None) -> Tuple[List[np.ndarray], List[Optional[str]]]:
...
def dataset_making(folder_path: str) -> None:
